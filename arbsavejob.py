...
def scrape_and_save_job_info(url, excel_filename='+Applied jobs.xlsx'):
    ...
    try:
        ...
        new_row = {'Job Name': job_name, 'Company Name': company_name, 'Job Type': job_type, 'Location': location}
        
        # Check if the Excel file already exists
        if os.path.exists(excel_filename):
            df = pd.read_excel(excel_filename)
        else:
            df = pd.DataFrame(columns=['Job Name', 'Company Name', 'Job Type', 'Location'])
